...
{
    char **buff = value;
    return (ippAddAttr(req_attr, name, (void **)buff, 10, group_tag, IPP_TAG_BEGIN_COLLECTION));
}

int ippAddKeyword(ipp_attribute **req_attr, char *name, void *value, int vale_num, ipp_tag_t group_tag)
{
    char **buff = value;
    return (ippAddAttr(req_attr, name, (void **)buff, vale_num, group_tag, IPP_TAG_KEYWORD));
}
