...
typedef struct ipp_attribute_s            /**** IPP attribute ****/
{
  ipp_tag_t group_tag;
  ipp_tag_t value_tag;          /* What type of value is it? */
  short int name_len;           /* Name of attribute */
  char      *name;              /* Name of attribute */
  short int values_count;       /* Number of values */
  ipp_value_t *values;
  struct ipp_attribute_s *next;
} ipp_attribute;

#define pp_malloc(x)       malloc(x)
#define pp_calloc(x, size) calloc(x, size)
#define pp_free(x)         free(x)

int ippAddAttr(ipp_attribute **req_attr, char *name, 
               void **value, int values_count, ipp_tag_t group_tag, ipp_tag_t value_tag);
...
int ippAddKeyword(ipp_attribute **req_attr, char *name, void *value, int vale_num, ipp_tag_t group_tag);

void ippReleaseAttr(ipp_attribute **req_attr);
int ippAttrToReqBuff(ipp_attribute *req_attr,
